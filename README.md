...
添加PWA功能,IOS/安卓都能通用,IOS需要手动添加到屏幕才支持息屏播放，安卓以及鸿蒙,会自动提示安装，安装体验效果更新
...
1. **登录后台**：访问 `/admin` 路径，使用管理员账号登录
2. **进入接口管理**：在后台导航栏中点击"接口管理"标签
3. **点击添加新接口**：在接口管理页面点击"添加新接口"按钮
4. **填写基本信息**：
   - 接口名称：唯一的接口标识符（如 `my_api`）
   - 显示名称：在前端显示的接口名称（如 `我的自定义接口`）
   - 描述：接口的简要说明
5. **配置字段映射**：
   - **搜索接口映射**：配置搜索结果中各字段的映射关系
     - 书籍ID字段路径
     - 书籍标题字段路径
     - 书籍封面字段路径
     - 主播名字段路径
     - 章节列表URL字段路径
   - **章节列表映射**：配置章节列表中各字段的映射关系
     - 章节ID字段路径
     - 章节标题字段路径
     - 章节时长字段路径
     - 音频URL字段路径
   - **音频URL映射**：配置音频URL的获取方式
     - 直接使用字段值
     - 使用模板URL
6. **保存配置**：点击"保存"按钮保存接口配置
7. **启用接口**：在接口列表中启用新添加的接口
8. **测试接口**：在前台搜索页面选择新接口进行测试

**字段路径说明**：
- 字段路径用于从接口返回的JSON数据中提取对应字段
- 使用点号（.）表示嵌套对象，如 `data.books`
- 使用方括号（[]）表示数组索引，如 `data.books[0]`
- 支持使用通配符（*）表示遍历数组，如 `data.books[*]`
